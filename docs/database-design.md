...
| date                  | DateTime | 訓練日期                                              |
| sportCategory         | String   | 可選，標準運動分類；休息日或尚待確認的舊資料可為空值  |
...
| sportCategory    | String   | 可選，標準運動分類；休息紀錄可為空值      |
...
`TrainingDay` 與 `WorkoutLog` 共用的標準運動分類。`trainingType` 與 `workoutType` 仍保留詳細訓練內容，不以分類欄位取代。
...
## 4.7 TrainingPlanConversation.readiness
